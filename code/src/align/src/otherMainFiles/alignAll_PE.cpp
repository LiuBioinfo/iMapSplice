		...
		; tmpTurn++)
	{
		...
		for(int tmpOpenMP = 0; tmpOpenMP < realRecordNum; tmpOpenMP++)
		{
			...
			readInfo.initiateReadInfo(readName1Vec[tmpOpenMP], readName2Vec[tmpOpenMP],
				...
				readQualSeq1Vec[tmpOpenMP], readQualSeq2Vec[tmpOpenMP], fasta_or_fastq_bool);
			
			// cout << "read_name_1: " << readName1Vec[tmpOpenMP] << endl;
			...
			fixPhase1Info.fixPhase1_segInfo(
				...
				readInfo, repeatRegionInfoVec[threadNO], checkQualSeqForReadSegSeq);
		...
		if(!outputDirectlyBool_Phase1Only)
		{
			...
			{	
				if(PeAlignSamStrVec_complete[tmp] != "")
				{
					...
					PairedReadNum ++;
					string().swap(PeAlignSamStrVec_complete[tmp]);
					//PeAlignSamStrVec_complete[tmp] = "";
				}			
				if(PeAlignInfoStrVec_inCompletePair[tmp] != "")
				{
					...
					string().swap(PeAlignInfoStrVec_inCompletePair[tmp]);
				}
				if(PeAlignInfoStrVec_oneEndUnmapped[tmp] != "")
				{
					...
					string().swap(PeAlignInfoStrVec_oneEndUnmapped[tmp]);
				}
				if(PeAlignSamStrVec_bothEndsUnmapped[tmp] != "")
				{
					...
					BothUnmappedReadNum ++;
					string().swap(PeAlignSamStrVec_bothEndsUnmapped[tmp]);
				}
				if(PeAlignSamStrVec_bothEndsUnmapped_lowScore[tmp] != "")
				{
					...
					//PeAlignSamStrVec_bothEndsUnmapped_lowScore[tmp] = "";
					string().swap(PeAlignSamStrVec_bothEndsUnmapped_lowScore[tmp]);
				}
				if(PeAlignSamStrVec_bothEndsUnmapped_mappedToRepeatRegion[tmp] != "")
				{
					...
					//PeAlignSamStrVec_bothEndsUnmapped_mappedToRepeatRegion[tmp] = "";
					string().swap(PeAlignSamStrVec_bothEndsUnmapped_mappedToRepeatRegion[tmp]);
				}
				if(PeAlignSamStrVec_inCompletePair[tmp] != "")
				{
					tmpAlignIncompletePair_SAM_ofs << PeAlignSamStrVec_inCompletePair[tmp] << endl;
					//PeAlignSamStrVec_inCompletePair[tmp] = "";
					string().swap(PeAlignSamStrVec_inCompletePair[tmp]);
				}
			}
		}
		#ifdef CAL_TIME
