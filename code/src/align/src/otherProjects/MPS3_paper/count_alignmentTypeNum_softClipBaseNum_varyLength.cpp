...
int main(int argc, char** argv)
{
	if(argc != 6)
	{
		cout << "Executable TotalReadNum ReadLengthMax SamFile outputFolder methodName" << endl;
		exit(1);
	}
	...
	log_ofs << "readLengthMax: " << readLengthMax << endl;
		...
		else
		{
			...
			{	
				if(primaryOrNot_bool)
				{
					...
					int softClippedBothEndsLength = softClippedHeadLength + softClippedTailLength;
					//int mappedBaseNum_justified2readLengthMax = readLengthMax - softClippedBothEndsLength;
					// for(int tmpBaseLoc = startMappedBaseLocInRead; 
					// 		tmpBaseLoc <= endMappedBaseLocInRead; tmpBaseLoc++)
					for(int tmpBase = softClippedBothEndsLength + 1; 
						tmpBase <= readLengthMax; tmpBase ++)
						mappedBaseNumVec_primaryAlignment[tmpBase-1] ++;
				}
				else
				{}
			}
