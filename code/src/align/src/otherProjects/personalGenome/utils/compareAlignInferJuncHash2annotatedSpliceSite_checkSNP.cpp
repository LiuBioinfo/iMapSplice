...
int main(int argc, char** argv)
{
	if(argc != 6)
	{
		cout << "Executable <InputIndexFolderPath> <inputSNPfile> <SJfromAnnotationFile> <ToCompareJuncFile> <outputFolder> <inputSNPfile>" << endl;
		exit(1);
	}
	int fixedAnchorSize = 30;
	...
	log_ofs << "inputSNPfile: " << argv[2] << endl;
	log_ofs << "SJfromAnnotationFile: " << argv[3] << endl;
	log_ofs << "ToCompareJuncFile: " << argv[4] << endl;
	...
	string outputReformattedSNPfile = outputFolderStr + "reformattedSNP.txt";
	tmpSNPhashInfo.output(outputReformattedSNPfile, indexInfo);
	...
	string annotated_oneEnd_JuncFile_withOtherJuncInfo_withSNP = outputFolderStr + "annotated_oneEnd_detailInfo_withSNP.junc";
	...
	string juncHash_file_1_str = argv[3];
	...
	ofstream oneEndAnnotated_detail_withSNP_ofs(annotated_oneEnd_JuncFile_withOtherJuncInfo_withSNP.c_str());
	ofstream unannotated_ofs(unannotated_JuncFile.c_str());
	string juncHash_file_2_str = argv[4];
		...
		else if(startPosFoundBool || endPosFoundBool)
		{
			...
			if(startPosFoundBool)
			{
				...
				for(int tmp = 0; tmp < tmpAlterAcceptorSpliceSiteVec.size(); tmp++)
				{
					...
					tmpSNPhashInfo.returnSNPvecWithinRegion(tmpChrNameInt, 
						tmpStartPos - fixedAnchorSize + 1, tmpStartPos,
						tmpSNPposVec, tmpSNPbaseVec);
					if(tmpSNPposVec.size() > 0)
					{
						oneEndAnnotated_detail_withSNP_ofs << "AlterSpliceJunc:";
						oneEndAnnotated_detail_withSNP_ofs << "\t" << tmpStartPos 
							<< ":" << tmpAlterAcceptorSpliceSiteVec[tmp] << endl;
						oneEndAnnotated_detail_withSNP_ofs << tmpStr << endl;
					}
				}
			}
			else
			{
				...
				for(int tmp = 0; tmp < tmpAlterDonerSpliceSiteVec.size(); tmp++)
				{
					...
					vector<string> tmpSNPbaseVec;
					tmpSNPhashInfo.returnSNPvecWithinRegion(tmpChrNameInt, 
						tmpEndPos, tmpStartPos + fixedAnchorSize - 1,
						tmpSNPposVec, tmpSNPbaseVec);
					if(tmpSNPposVec.size() > 0)
					{
						oneEndAnnotated_detail_withSNP_ofs << "AlterSpliceJunc:";
						oneEndAnnotated_detail_withSNP_ofs << "\t" 
							<< tmpAlterDonerSpliceSiteVec[tmp] << ":" << tmpEndPos << endl;
						oneEndAnnotated_detail_withSNP_ofs << tmpStr << endl;
					}
				}
			}
