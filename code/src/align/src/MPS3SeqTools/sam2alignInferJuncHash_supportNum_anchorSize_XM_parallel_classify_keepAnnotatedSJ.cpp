...
int main(int argc, char**argv)
{
	if(argc < 4)
	{
		cout << "Executable <InputIndexInfo> <threadNum> <annotatedSJ> <OutputSJ> <inputSAM_1> ... (other input SAM files)" << endl;
		exit(1);
	}
	...
	local = localtime(&nowtime);
	cout << endl << "[" << asctime(local) << "...";
	cout << "start to initaite alignInferJunc from annotation " << endl;
	log_ofs << endl << "[" << asctime(local) << "...";
	log_ofs << "start to initaite alignInferJunc from annotation " << endl;
	string annotatedSJpath = argv[3];
	AlignInferJunctionHash_Info* alignInferJunctionHashInfo_ann = new AlignInferJunctionHash_Info();
	alignInferJunctionHashInfo_ann->initiateAlignInferJunctionInfo(chromNum);
	alignInferJunctionHashInfo_ann->insertJuncFromJuncFile_chrNamePosOnly(annotatedSJpath, indexInfo);
	...
	for(int tmp = 0; tmp < alignInferInfoVecSize; tmp++)
	{
		...
		int searchAndReturnedIndexInAnnotatedSJhash
			= alignInferJunctionHashInfo_ann->searchAndReturnAlignInferInfoVecIndex(
				tmpSJ_chrNameInt, tmpSJ_donerEndPos, tmpSJ_acceptorStartPos);
		bool SJannotatedOrNot_bool = (searchAndReturnedIndexInAnnotatedSJhash >= 0);
		if(SJannotatedOrNot_bool)
			tmpJuncStr += "ANNOTATED\t";
		else
			tmpJuncStr += "UNANNOTATED\t";

		if(((lowSupportNumNonCanonical_bool || matchThroughSeqSimilar_bool || alterSpliceSiteSeqSimilar_bool))&&(!SJannotatedOrNot_bool))
		{
	...
	delete alignInferJunctionHashInfo_ann;
	delete indexInfo;
	//sj_ofs.close();
	parameter_ifs.close();
	return 0;
