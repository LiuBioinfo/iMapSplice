...
void extractXMfromFusionJuncStr(int& tmpXMmin, int& tmpXMmax, string& tmpFusionJuncStr)
{
	...
	tmpXMmin = atoi(tmpFusionJuncFieldVec[11].c_str());
	tmpXMmax = atoi(tmpFusionJuncFieldVec[12].c_str());
}
...
int main(int argc, char** argv)
{
	if(argc != 4)
	{
		cout << "Executable inputFusionJunc outputPrefix maxReadLength" << endl;
		exit(1);
	}
	...
	while(!fusionJunc_ifs.eof())
	{
		...
		extractXMfromFusionJuncStr(tmpXMmin, tmpXMmax, tmpFusionJuncStr);
