...
void checkAlterSpliceAnchorSeqSimilarity(
	...
	vector<int>& supportNum_alterAcceptor,
	vector<int>& supportNum_alterDoner,
	int toCheckAnchorLengthMax, Index_Info* indexInfo)
{
	...
	for(int tmp = 0; tmp < tmpAlterAcceptorSpliceSitePosVec.size(); tmp++)
	{
		...
		else
		{
			...
			int tmpAlterAcceptorSite_supportNum
				= alignInferJuncHashInfo->returnAlignInferInfo_supportNum(tmpIndexInAlignInferJuncVec);
			supportNum_alterAcceptor.push_back(tmpAlterAcceptorSite_supportNum);
		}
	}
	...
	for(int tmp = 0; tmp < tmpAlterDonerSpliceSitePosVec.size(); tmp++)
	{
		...
		else
		{
			...
			int tmpAlterDonerSite_supportNum
				= alignInferJuncHashInfo->returnAlignInferInfo_supportNum(tmpIndexInAlignInferJuncVec);
			supportNum_alterDoner.push_back(tmpAlterDonerSite_supportNum);
		}
	}
}		
...
int main(int argc, char**argv)
{
	if(argc < 6)
	{
		cout << "Executable <InputIndexInfo> <inputFormattedSNPfile> <threadNum> <OutputSJ> <inputSAM_1> ... (other input SAM files)" << endl;
		exit(1);
	}
	...
	for(int tmp = 0; tmp < alignInferInfoVecSize; tmp++)
	{
		...
		checkAlterSpliceAnchorSeqSimilarity(
			...
			supportNum_alterAcceptor, supportNum_alterDoner,
			toCheckAnchorLengthMax, indexInfo);
		...
		else
		{
			...
			for(int tmp = 0; tmp < validAnchorLengthVec_alterAcceptor.size(); tmp++)
			{
				...
				int tmpSupportNum = supportNum_alterAcceptor[tmp];
				if((tmpSJ_supportNum < tmpSupportNum)
					&&(inconfidentSJ_anchorSeqSimilarity(tmpPenalty, tmpValidAnchorLength)))
		...
		else
		{
			...
			for(int tmp = 0; tmp < validAnchorLengthVec_alterDoner.size(); tmp++)
			{
				...
				int tmpSupportNum = supportNum_alterDoner[tmp];
				if((tmpSJ_supportNum < tmpSupportNum)
					&&(inconfidentSJ_anchorSeqSimilarity(tmpPenalty, tmpValidAnchorLength)))
					alterSpliceSiteSeqSimilar_bool = true;
				tmpJuncStr = tmpJuncStr + int_to_str(tmpAlterSite) + ":"
